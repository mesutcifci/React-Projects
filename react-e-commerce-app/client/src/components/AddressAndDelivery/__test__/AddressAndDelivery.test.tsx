...
describe("AddressDataAndDelivery test", () => {
  ...
  /**
   * Since I migrated from jest to vitest the following test have started to
   * affect the other two. So I commented the problematic tests
   */
  ...
  //     await waitFor(() =>
  //       expect(screen.getByText(data.errorMinLength)).toBeInTheDocument()
  //     );

  //     fireEvent.change(nameInputElement, { target: { value: "ab1" } });
  //     await waitFor(() =>
  //       expect(screen.getByText(data.errorMinLength)).toHaveTextContent(
  //         data.errorContainNumber
  //       )
  //     );
  //   });
  ...
  test("Should be able to type in inputs", () => {
    inputData.forEach(async (data) => {
      ...
      if (data.label === "Phone Number") {
        ...
        await waitFor(() => expect(inputElement.value).toBe("Turkey"));
